...
const db = {
  ...
  async checkCreditLimits(userId) {
    if (!supabase) {
      // Fallback limits when no database - TESTING LIMITS
      return {
        isRegistered: !!userId && userId !== 'anonymous',
        dailyLimit: !!userId && userId !== 'anonymous' ? 100 : 50,
        currentUsage: 0,
        remaining: !!userId && userId !== 'anonymous' ? 100 : 50,
        canConvert: true
      };
    }
    ...
    if (subscription) {
      ...
      // INCREASED FOR TESTING - Revert before public launch
      switch (subscription.plan_type) {
        case 'starter': dailyLimit = 500; break;           // Was 50
        case 'professional': dailyLimit = 2000; break;     // Was 200
        default: dailyLimit = isRegistered ? 100 : 50;     // Was 5 : 1
      }
    } else {
      // Free users - TESTING LIMITS (was 5:1)
      dailyLimit = isRegistered ? 100 : 50;
    }
    
    // Get today's usage from conversions table (each conversion = 1 credit)
    const { data: todayConversions, error } = await supabase
      .from('conversions')
      .select('id')
      .eq('user_id', uuid)
      .gte('created_at', `${today}T00:00:00`)
    ...
    const currentUsage = (todayConversions || []).length;
    ...
    return {
      ...
      console.error('Error in checkCreditLimits:', error);
      // On error, return generous limits to allow testing - TESTING ONLY
      return {
        isRegistered: !!userId && userId !== 'anonymous',
        dailyLimit: !!userId && userId !== 'anonymous' ? 100 : 50,
        currentUsage: 0,
  ...
  // Track credit usage
  async trackCreditUsage(usageData) {
    if (!supabase) {
      throw new Error('Supabase not configured');
    }
    
    const uuid = googleIdToUuid(usageData.userId);
    const { data, error } = await supabase
      .from('credit_usage')
      .insert([{
        user_id: uuid,
        file_name: usageData.fileName,
        page_count: usageData.pageCount || 1,
        credits_used: usageData.creditsUsed || 1,
        usage_date: usageData.date || new Date().toISOString(),
        description: usageData.description || 'PDF conversion'
      }])
      .select()
      .single();
    ...
    console.log('✅ Credit usage tracked successfully:', data);
    return data;
  }
};
